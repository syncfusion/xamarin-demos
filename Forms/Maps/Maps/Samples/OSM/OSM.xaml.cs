...
namespace SampleBrowser.SfMaps
{
    ...
    public partial class OSM : SampleView
    {
                ...
                childGrid.Children.Clear();
            {
                for (int i = 0; i < childGrid.RowDefinitions.Count; i++)
                {
                    for (int j = 0; j < childGrid.ColumnDefinitions.Count; j++)
                    {
                        ...
                        {
#if COMMONSB
                               image.Source = ImageSource.FromResource("SampleBrowser.Icons.grid.png", typeof(OSM).GetTypeInfo().Assembly);
#else
                              image.Source = ImageSource.FromResource("SampleBrowser.SfMaps.Icons.grid.png", typeof(OSM).GetTypeInfo().Assembly);
#endif
                        }
                        else
                        {
#if COMMONSB
                            image.Source = ImageSource.FromResource("SampleBrowser.Icons.grid.png");
#else
                            image.Source = ImageSource.FromResource("SampleBrowser.SfMaps.Icons.grid.png");
#endif
                        }
        ...
        {
            var netConnection = DependencyService.Get<OSMINetwork>();
            if (netConnection != null)
            {
                await netConnection.CheckNetworkConnection();
    ...
    public interface OSMINetwork
    {
        bool IsConnected { get; }
        Task CheckNetworkConnection();
    }
