...
namespace SampleBrowser.SfDiagram
{
    ...
    public partial class MindMap : SampleView
    {
        ...
        private void AddHandles()
        {
            ...
            m_expandTemplate = GetTemplate("collpase.png");
            m_collapseTemplate = GetTemplate("expand.png");
        ...
        private DataTemplate GetTemplate(string imageId)
        {
            return new DataTemplate(() =>
            {
                ...
                if (Device.RuntimePlatform == Device.iOS)
                    image.Source = path + imageId;
                else if (Device.RuntimePlatform == Device.Android)
                    image.Source = path + "mindmap" + imageId;
        ...
        void ShowInfo()
        {
            ...
            noteGrid.ColumnSpacing = 20;
            noteGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Auto) });
            noteGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Auto) });
            noteGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Auto) });
            noteGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Auto) });
            noteGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Auto) });
            noteGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Auto) });
            noteGrid.HeightRequest = 70;
            ...
            title.VerticalOptions = new LayoutOptions() { Alignment = LayoutAlignment.Start };
            title.HorizontalOptions = new LayoutOptions() { Alignment = LayoutAlignment.Start };
            noteGrid.Children.Add(title, 0, 0);

            Button cancel = new Button();
            cancel.Text = "CANCEL";
            cancel.BackgroundColor = Color.Transparent;
            cancel.Clicked += Cancel_Clicked1;
            cancel.HorizontalOptions = LayoutOptions.End;

            noteGrid.Children.Add(cancel, 5, 0);

            Button ok = new Button();
            ok.Text = " OK";
            ok.BackgroundColor = Color.Transparent;
            ok.Clicked += Ok_Clicked1;
            ok.HorizontalOptions = LayoutOptions.End;
            noteGrid.Children.Add(ok, 6, 0);
            root.Children.Add(noteGrid);

            CommentBoxEntry = new Editor();
            CommentBoxEntry.HeightRequest = parent.Height - 110;
            ...
            CommentBoxEntry.VerticalOptions = LayoutOptions.EndAndExpand;
            root.Children.Add(CommentBoxEntry);

            InfoNotifier = new Frame();
            InfoNotifier.WidthRequest = parent.Width;
            InfoNotifier.HeightRequest = parent.Height;
            ...
            InfoNotifier.BorderColor = (SelectedNode.Style.Brush as SolidBrush).FillColor;
            parent.Children.Add(InfoNotifier);
        }

        private void Cancel_Clicked1(object sender, EventArgs e)
        {
            parent.Children.Remove(InfoNotifier);
        }
