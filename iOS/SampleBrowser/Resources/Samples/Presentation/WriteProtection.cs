...
namespace SampleBrowser
{
	public partial class WriteProtection : SampleView
	{
		...
		void LoadAllowedTextsLabel()
		{
			...
			label.Text = "This sample demonstrates how to set a write protection for a PowerPoint presentation.";
            ...
            passwordLabel.Frame = frameRect;
            passwordLabel.TextColor = UIColor.FromRGB(38 / 255.0f, 38 / 255.0f, 38 / 255.0f);
            passwordLabel.Text = "Password: syncfusion";
            ...
            if (UIDevice.CurrentDevice.UserInterfaceIdiom == UIUserInterfaceIdiom.Pad)
            {
                ...
                passwordLabel.Frame = new CGRect(5, 55, frameRect.Location.X + frameRect.Size.Width, 50);
            }
            else
            {
                passwordLabel.Frame = new CGRect(frameRect.Location.X, 75, frameRect.Size.Width, 70);

            }
            this.AddSubview(passwordLabel);


            button.SetTitle("Generate Presentation",UIControlState.Normal);
			if (UIDevice.CurrentDevice.UserInterfaceIdiom == UIUserInterfaceIdiom.Pad) {
				button.Frame = new CGRect (0, 105, frameRect.Location.X + frameRect.Size.Width , 10);
			} 
			else {
				button.Frame = new CGRect (frameRect.Location.X, 150, frameRect.Size.Width , 10);
		...
		void OnButtonClicked(object sender, EventArgs e)
		{
			...
			if (stream != null)
			{
				...
				iOSSave.Save ("WriteProtectionSample.pptx", "application/mspowerpoint", stream);
			}
