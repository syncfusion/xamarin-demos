...
namespace SampleBrowser
{
    ...
    public class MapsDrilldownDelegate : SFMapsDelegate
    {
        public MapsDrilldownDelegate(Drilldown sample)
        {
            mapping = sample;
        }
        Drilldown mapping;
        ...
        public override void DidSelectShape(SFMap map, NSObject data)
        {
            if (data != null)
            {
                ...
                mapping.label3.Text = continent;
                mapping.header.Hidden = true;
                mapping.layout.Hidden = false;
                mapping.button.Hidden = false;
                if (continent == "South America")
                {
                    mapping.maps.BaseMapIndex = 1;
                    mapping.layer.ShapeSettings.SelectedShapeColor = UIColor.FromRGB(156, 51, 103);
                }
                else if (continent == "North America")
                {
                    mapping.maps.BaseMapIndex = 2;
                    mapping.layer.ShapeSettings.SelectedShapeColor = UIColor.FromRGB(193, 54, 100);
                }
                else if (continent == "Europe")
                {
                    mapping.maps.BaseMapIndex = 3;
                    mapping.layer.ShapeSettings.SelectedShapeColor = UIColor.FromRGB(98, 45, 108);
                }
                else if (continent == "Africa")
                {
                    mapping.maps.BaseMapIndex = 4;
                    mapping.layer.ShapeSettings.SelectedShapeColor = UIColor.FromRGB(128, 48, 106);
                }
                else if (continent == "Australia")
                {
                    mapping.maps.BaseMapIndex = 5;
                    mapping.layer.ShapeSettings.SelectedShapeColor = UIColor.FromRGB(42, 40, 112);
                }
                else if (continent == "Asia")
                {
                    mapping.maps.BaseMapIndex = 6;
                    mapping.layer.ShapeSettings.SelectedShapeColor = UIColor.FromRGB(70, 42, 109);
                }
            }
        }
    }
