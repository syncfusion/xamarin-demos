	...
	public class TextCollectionDataSource : UICollectionViewDataSource
	{
		...
		public void HightlightCell(UIButton label, UIButton labelX, UICollectionView collectionView, NSIndexPath indexPath)
		{
			if (controller.indexPath != indexPath)
			{
				...
				if (control.ControlName == "DataGrid") 				{
					label.Alpha = 0.5f;
					labelX.Alpha = 0.5f;
 					NSTimer.CreateScheduledTimer(new TimeSpan(0, 0, 0, 0, 2), delegate 					{
						label.Alpha = 1f;
						labelX.Alpha = 1f; 						controller.LoadSample(); 					}); 				} 				else 					controller.LoadSample();
