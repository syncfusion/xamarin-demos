    ...
    public class AllControlsCollectionDelegate : UICollectionViewDelegate
	{
        ...
        public override void ItemSelected(UICollectionView collectionView, NSIndexPath indexPath)
		{
			...
			for (nuint i = 0; i < dictArray.Count; i++)
			{
				...
				if (dict.ValueForKey(new NSString("IsNew")) != null && dict.ValueForKey(new NSString("IsNew")).ToString().ToUpper() == "YES")
				{
					control.Tag = new NSString("NEW");
				}
				else if (dict.ValueForKey(new NSString("IsUpdated")) != null && dict.ValueForKey(new NSString("IsUpdated")).ToString().ToUpper() == "YES")
				{
					control.Tag = new NSString("UPDATED");
				}
				else if (dict.ValueForKey(new NSString("IsPreview")) != null && dict.ValueForKey(new NSString("IsPreview")).ToString().ToUpper() == "YES")
				{
