...
namespace SampleBrowser
{
	...
	public class IncrementalLoadingViewModel : INotifyPropertyChanged
	{
			...
			set {
				...
				if (progress != null) {
					if (isBusy) {
						...
						if (noNetwork) {
							progress.SetMessage ("No Network Found..! \n\nSearching for a network...");
							progress.Show ();
						} else
							progress.Hide ();
					}
		...
		public class MyProgressDialog : ProgressDialog
		{
			#region Constructor

			public MyProgressDialog (Context context) : base (context)
			{
			...
			public override bool OnKeyDown (Keycode keyCode, KeyEvent e)
			{
				...
				//return currentActivity.OnKeyDown(keyCode, e);
