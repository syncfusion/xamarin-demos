...
namespace SampleBrowser
{
    public class DataPoint
    {
        ...
        public DataPoint(object xVal, double yVal, double horizontalValues, double verticalValues)
        {
            XValue = xVal;
            YValue = yVal;
            High = horizontalValues;
            Low = verticalValues;
        }

        public DataPoint(string department, List<double> ages)
        {
    ...
    public class MainPage
    {
        ...
        public static List<DataPoint> GetErrorBarData()
        {
            var datas = new List<DataPoint>();
            datas.Add(new DataPoint("IND", 23, 0.5, 1));
            datas.Add(new DataPoint("AUS", 20, 0, 2));
            datas.Add(new DataPoint("USA", 35, 1, 2));
            datas.Add(new DataPoint("DEU", 28, 2, 0.5));
            datas.Add(new DataPoint("ITA", 30, 1, 0));
            datas.Add(new DataPoint("UK", 42, 1.5, 1));
            datas.Add(new DataPoint("RUS", 27, 0.5, 2));
            return datas;
        }

        public static List<DataPoint> BoxAndWhiskerData()
        {
            var BoxAndWhiskerData = new List<DataPoint>()
            {
                new DataPoint("Development", new List<double> { 22, 22, 23, 25, 25, 25, 26, 27, 27, 28, 28, 29, 30, 32, 34, 32, 34, 36, 35, 38 }),
                new DataPoint("Testing", new List<double> { 22, 33, 23, 25, 26, 28, 29, 30, 34, 33, 32, 31, 50 }),
                new DataPoint("HR", new List<double> { 22, 24, 25, 30, 32, 34, 36, 38, 39, 41, 35, 36, 40, 56 }),
                new DataPoint("Finance", new List<double> { 26, 27, 28, 30, 32, 34, 35, 37, 35, 37, 45 }),
                new DataPoint("Sales", new List<double> { 26, 27, 29, 32, 34, 35, 36, 37, 38, 39, 41, 43, 58 }),
            };

            return BoxAndWhiskerData;
        }

        public static ObservableCollection<DataPoint> GetTrendlineDataSource1()
        {
            var yValue = new double[] { 10.11, 11.36, 12.34, 12.60, 12.95, 13.91, 16.21, 17.50, 22.72, 28.14, 31.26, 31.39, 32.43, 35.52, 36.36, 41.33, 43.12, 45.00, 47.23, 48.62, 46.60, 45.28, 44.01, 45.17, 41.20, 43.41, 48.32, 45.65, 46.61, 53.34, 58.53, 59.02, 59.32, 61.24, 62.32, 62.43, 64.86, 63.42, 65.93, 66.99, 67.02, 68.43, 71.01 };
            DateTime date = new DateTime(1977, 01, 01);
            var datas = new ObservableCollection<DataPoint>();
            foreach (var y in yValue)
            {
                datas.Add(new DataPoint(date, y));
        ...
        public static ObservableCollection<DataPoint> GetTrendlineDataSource2()
        {
            var powerData = new ObservableCollection<DataPoint>();
            powerData.Add(new DataPoint(1, 10));
            powerData.Add(new DataPoint(2, 50));
            powerData.Add(new DataPoint(3, 80));
            powerData.Add(new DataPoint(4, 110));
            powerData.Add(new DataPoint(5, 180));
            powerData.Add(new DataPoint(6, 220));
            powerData.Add(new DataPoint(7, 300));
            powerData.Add(new DataPoint(8, 370));
            powerData.Add(new DataPoint(9, 490));
            powerData.Add(new DataPoint(10, 500));
